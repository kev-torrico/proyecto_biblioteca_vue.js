  ...
  <div class="container py-4">
    ...
    <div class="table-responsive shadow-sm rounded">
      <table class="table table-hover align-middle">
        ...
        <tbody>
          <tr v-for="item in autoresFiltrados" :key="item.id">
            <td>{{ item.id }}</td>
            <td>{{ item.nombre }}</td>
        ...
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title text-primary" id="modalAutoEditarLabel">
              {{ modalMode === "crear" ? "Crear Autor" : "Editar Autor" }}
            </h5>
            <button
              type="button"
              class="btn-close"
...
<script>
import AutoresCrear from "./AutoresCrear.vue";
import AutoresEditar from "./AutoresEditar.vue";
export default {
  name: "AutoresList",
  data() {
    return {
  ...
  methods: {
    ...
    nuevo(value) {
      this.axios
        .post(process.env.VUE_APP_API_URL + "/autores", value)
        .then((response) => {
          this.cerrarModal();
          this.obtenerLista();
        })
    ...
    modificar(value) {
      this.axios
        .patch(process.env.VUE_APP_API_URL + "/autores/" + value.id, value)
        .then((response) => {
          this.cerrarModal();
          this.obtenerLista();
        })
    ...
    eliminar(value) {
      if (confirm("¿Está seguro de eliminar este ítem?")) {
        this.axios
          .delete(process.env.VUE_APP_API_URL + "/autores/" + value.id, value)
          .then((response) => {
            this.obtenerLista();
          })
          .catch((error) => {
  ...
  computed: {
    autoresFiltrados() {
      if (!this.textoABuscar) return this.items;
